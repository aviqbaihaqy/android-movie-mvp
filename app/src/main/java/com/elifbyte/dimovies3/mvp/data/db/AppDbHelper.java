...
import com.elifbyte.dimovies3.mvp.data.db.model.DaoMaster;
import com.elifbyte.dimovies3.mvp.data.db.model.DaoSession;
import com.elifbyte.dimovies3.mvp.data.db.model.Favorite;
import com.elifbyte.dimovies3.mvp.data.db.model.User;
import com.elifbyte.dimovies3.mvp.data.provider.FavoriteProvider;
...
public class AppDbHelper implements DbHelper {
    ...
    @Override
    public Observable<Boolean> deleteFavorite(final Favorite favorite) {
        return Observable.fromCallable(new Callable<Boolean>() {
            @Override
            public Boolean call() {
                mDaoSession.getFavoriteDao().delete(favorite);
                return true;
            }
        });
    }
    ...
    @Override
    public Observable<Favorite> getFavoriteById(final Long id) {
        return Observable.fromCallable(new Callable<Favorite>() {
            @Override
            public Favorite call() {
              return mDaoSession.getFavoriteDao().loadByRowId(id);
            }
        });
    }
